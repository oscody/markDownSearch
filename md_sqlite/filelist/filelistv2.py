...
OUTPUT_FILENAME = "file_list_order.md"
...
if __name__ == "__main__":
    # Get the list of Markdown files from the allowed directories
    md_files = list_md_files(START_DIRECTORY, allowed_dirs)
    
    # Sort the files by creation time in descending order (newer files first)
    sorted_md_files = sorted(md_files, key=lambda f: os.path.getctime(f), reverse=True)
    
    write_to_markdown(sorted_md_files, OUTPUT_FILENAME)